...
export const useAnalytics = () => {
  ...
  const trackPageView = useCallback(async (page: string, _properties?: Record<string, any>) => {
  ...
  const trackTiming = useCallback(async (category: string, variable: string, _time: number, _label?: string) => {
  ...
  const trackInteraction = useCallback(async (element: string, action: string, _value?: unknown) => {
