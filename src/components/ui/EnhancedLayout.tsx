      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            $match = import { ReactNode, useEffect, useState } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { 
  Menu, X, AlertTriangle, Search, User, Settings, LogOut, 
...
function EnhancedLayoutContent({ children }: EnhancedLayoutProps) {
  ...
  const [__isInstallable, _setIsInstallable] = useState(false);
  const [deferredPrompt, _setDeferredPrompt] = useState<unknown>(null);
  ...
  return (
    <NavigationProvider>
      <EnhancedLayoutContent>{children}</EnhancedLayoutContent>
            ...
            return import { ReactNode, useEffect, useState } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { 
  Menu, X, AlertTriangle, Search, User, Settings, LogOut, 
  ...
  return (
    <NavigationProvider>
      <EnhancedLayoutContent>{children}</EnhancedLayoutContent>
         ...
         { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); () => setIsOpen(false)(e); } }} role="button" tabIndex={0}> setIsOpen(false)}
            />
